...
编译操作必须遵循**渐进式披露**原则，避免不必要的全文读取：

| 级别 | Token 预算 | 内容 | 使用时机 |
|------|------------|------|----------|
| L0 | ~200 | wiki/index.md 的章节结构 | 每次编译开始 |
| L1 | ~50/文件 | raw 文件的 frontmatter（前 15 行） | 判断编译状态 |
| L2 | ~100/文件 | wiki/concepts/*.md 的 frontmatter | 概念对标判断 |
| L3 | 全文 | 仅待编译的 raw 文件正文 | 生成摘要时 |

**关键纪律**：
- ❌ 不要读取所有 raw 文件全文来判断状态
- ❌ 不要读取所有概念文件全文来对标
- ✅ 先读 frontmatter，确认需要后才读全文
- ✅ 使用 `Read` 的 `limit` 参数限制读取行数
...
1. 读取 `wiki/index.md` 的"待编译"章节（如有）
2. 用 `Glob` 扫描 `raw/**/*.md` 获取文件列表
3. **只读 frontmatter**：对每个文件使用 `Read` 的 `limit: 15`，仅获取元数据
4. 筛选 `status: "pending"` 或无 status 字段的文件
5. 如果没有待编译文件，输出：`✅ 知识库已是最新，无需编译` 并退出

## 步骤 2：概念对标预扫描（L2）

**在读取待编译文件全文之前**，先判断可能涉及的概念：

1. 从待编译文件的 frontmatter 提取 `title` 和 `tags`
2. 读取 `wiki/index.md` 的"概念"章节，获取现有概念列表
3. **只读 frontmatter**：对相关概念文件使用 `Read` 的 `limit: 20`
4. 根据 frontmatter 的 `title`、`tags`、`derived_from` 判断关联关系
5. 记录：可能需要更新的概念列表
...
- [显示名称](summaries/文件名-summary.md) | [一句话总结，15-30字]
```
...
- [显示名称](concepts/概念名.md) | [一句话定义，15-30字]
```
...
**为什么加一句话总结？**
让 LLM 在 L0 级别（仅读 index.md）就能判断是否需要深入读取具体文件，避免不必要的全文加载。
