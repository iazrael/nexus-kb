...
### 双层架构：Wiki 索引层 + Raw 事实层

本系统采用双层配合架构：

1. **Wiki 层（索引/导航）**：编译后的结构化知识，便于快速定位和关联
2. **Raw 层（事实来源）**：原始资料存档，保证信息的完整性和可追溯性

**查询流程**：
```
用户提问 → 索引找 Wiki → 通过 Wiki 的 Source 找 Raw → 知识全面
```

Wiki 不是替代 Raw，而是作为**导航层**帮助用户快速定位原始资料。两者配合使用才能获得完整知识。
...
| 命令 | 验收标准 |
|------|---------|
| `/compile` | 正确生成内容、无失败即为成功 |
| `/health` | 评分 ≥ 80 为健康，< 80 认为有异常 |
| `/ask` | 用户主观判断满意度，不量化 |
